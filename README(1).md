...
### Missing key, authentication error, or quota failure

Check that `first_agent/.env` contains your key and that your project permits the requested API usage. Google's SDK also recognizes `GOOGLE_API_KEY`; [when both variables are set, it takes precedence][google-key-docs]. Check for an unintended key in your shell environment. Updating an SDK does not grant model access or increase account quotas.

### Managed-agent call fails, then a fallback runs

The code catches managed-agent exceptions in the main production-planning paths. Current [Antigravity documentation][google-antigravity] configures its underlying model through `agent_config`; several repository calls instead pass `model` alongside `agent`. Review those calls against the current API before relying on managed execution.

`director-agent` is referenced by the standalone director examples, but this repository does not include a script to create or register that agent. A failed managed call may therefore be followed by successful direct text generation. `first_agent.py` has no such fallback.

### Sketches are unavailable

The legacy Imagen endpoint is listed in Google's [model deprecation schedule][google-deprecations]. Restoring image generation requires updating `generate_scene_sketch()` in `server.py` and the separate `test_imagen.py` example. Google's [migration guidance][google-imagen] changes both the API method and response handling; replacing only the model-name string is not sufficient.

Even with a working image integration, the current code targets only scenes 1–3. Scenes 4–7 intentionally have no generated sketches.

### Sample video is missing or processing does not finish

Provide the local video required by the script and confirm the working directory. The upload-processing loop has no explicit timeout or failed-state handling; inspect the server terminal rather than relying on the animated browser messages. Production use needs bounded polling and explicit error handling.

### Raw text appears instead of scene cards

The server could not parse a JSON array from the model response. The UI displays the returned raw text when available. Stricter structured-output configuration and validation would make this path more robust.

## Limitations and deployment safety

**Treat this repository as a local prototype.** It has no authentication, saved-project database, production job queue, or implemented video-rendering pipeline. The landing page's comparisons and sample storyboards are static demonstrations, not measured analytics from the backend. Inspect generated scripts, timing, claims, and visual directions before using them in production.

**Do not expose the current server publicly without hardening it.** Debug mode is enabled, CORS is unrestricted, and the application directory is configured as the static folder. Move public assets into a dedicated directory so configuration files, source files, and uploads cannot be served unintentionally. Add authentication, rate limits, validated and sanitized upload filenames, file-type and size checks, request timeouts, and robust response validation.

**Reference uploads leave your machine.** The app saves video files under `uploads/` and sends them to Gemini. It does not implement automatic cleanup of local or remote uploads. Use reference material you are authorized to share, and define retention and deletion behavior before handling other people's files.

## Contributing

Keep changes focused and describe how they were checked. Useful starting points are migrating image generation, updating managed-agent calls, adding a pinned dependency manifest, improving upload safety, and introducing automated tests with mocked API responses. Do not commit real API keys, private videos, or generated artifacts containing sensitive information.

## License

The [original project README](first_agent/README.md) declares **MIT**, but the repository currently contains no `LICENSE` file. The full license text should be added to make the intended licensing explicit.

---

Documentation based on the repository's `main` branch as reviewed on September 10, 2026. Setup and API examples were checked against the source; live Gemini and image-generation calls were not executed during this documentation review.

[google-api-key]: https://aistudio.google.com/apikey
[google-key-docs]: https://ai.google.dev/gemini-api/docs/api-key
[google-sdk]: https://github.com/googleapis/python-genai
[google-antigravity]: https://ai.google.dev/gemini-api/docs/antigravity-agent
